...
export const Row = memo(({ item, refetch }: Props) => {
  ...
  return (
    ...
    >
      <div>{item.number}</div>
      <div>
        {Array.isArray(item._type) && item._type[0] === 'ColdWaterAreaMeter' ? (
          <HVS />
        ) : (
          <GVS />
        )}
      ...
      <div>
        {dayjs(item.installation_date).isValid()
          ? dayjs(item.installation_date).format('DD.MM.YYYY')
          : ''}
      </div>
      <div>{item.is_automatic ? 'Да' : 'Нет'}</div>
      <div>
